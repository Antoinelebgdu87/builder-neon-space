...
export function useFirebaseUserActivation() {
  ...
  const activateUser = async (userId: string, newUsername?: string, displayName?: string): Promise<void> => {
    if (!isOnline) {
    ...
    try {
      ...
      const updateData = {
        ...
        profile: {
          ...userData.profile,
          displayName: displayName || newUsername || userData.username || 'Utilisateur',
          ...(userData.profile || {})
        }
      };
      ...
      for (const docSnap of usersSnapshot.docs) {
        ...
        if (needsRepair) {
          const repairedData = {
            ...
            // Réparer le username
            username: data.username && data.username !== 'Compte supprimé' 
              ? data.username 
