...
export class FirebaseErrorHandler {
  ...
  static isBlocked(): boolean {
    ...
    if (this.isFirebaseBlocked && Date.now() - this.lastFailure > this.resetTimeout) {
      this.reset();
    }
  ...
  private static showUserNotification() {
    ...
    if (typeof window !== 'undefined') {
      ...
      notification.className = 'fixed top-4 right-4 z-[9999] bg-amber-500/90 text-white px-4 py-2 rounded-lg shadow-lg text-sm';
      notification.innerHTML = '⚠️ Mode hors ligne - Données locales uniquement';
  ...
  static reset() {
    ...
    this.lastFailure = 0;
    console.log('🔄 Firebase handler reset - Tentative de reconnexion autorisée');
  }
