...
export class EmergencyMode {
  ...
  private static showNotification() {
    if (typeof window !== 'undefined') {
      ...
      notification.className = 'fixed top-4 right-4 z-[9999] bg-blue-500/90 text-white px-4 py-2 rounded-lg shadow-lg text-sm max-w-sm';
      notification.innerHTML = '🚨 Mode local activé - Toutes les données sont sauvegardées localement';
