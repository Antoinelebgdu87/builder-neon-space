          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
            ...
            <TabsContent value="users" className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <Card className="glass border-border/50">
                  <CardContent className="p-6">
                    <div className="text-2xl font-bold text-primary">{bans.length}</div>
                ...
                <Card className="glass border-border/50">
                  <CardContent className="p-6">
                    <div className="text-2xl font-bold text-orange-400">
                      {bans.filter(ban => ban.banType === 'temporary').length}
                    </div>
                    <div className="text-sm text-muted-foreground">Bans temporaires</div>
                  </CardContent>
                </Card>
                <Card className="glass border-border/50">
                  <CardContent className="p-6">
                    <div className="text-2xl font-bold text-red-400">
                      {bans.filter(ban => ban.banType === 'permanent').length}
                    </div>
                    <div className="text-sm text-muted-foreground">Bans permanents</div>
                  </CardContent>
                </Card>
              </div>
                      ...
                      <div className="space-y-4">
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) => setBanFormData(prev => ({ ...prev, username: e.target.value }))}
                            placeholder="Anonyme123, Utilisateur456..."
                          />
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <div className="flex items-center space-x-4 mt-2">
                            <div className="flex items-center space-x-2">
                              ...
                              />
                              <label htmlFor="temporary" className="text-sm">Temporaire</label>
                            </div>
                            <div className="flex items-center space-x-2">
                              <input
                                type="radio"
              ...
              <Card className="glass border-border/50">
                ...
                <CardContent>
                  {bansLoading ? (
                    ...
                    <div className="space-y-4">
                      {bans.map((ban) => (
                        <div key={ban.userId} className="border border-border/30 rounded-lg p-4">
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <div className="flex items-center space-x-2 mb-2">
                                <h3 className="font-semibold">{ban.username}</h3>
                                <Badge variant={ban.banType === 'permanent' ? 'destructive' : 'secondary'}>
                                  {ban.banType === 'permanent' ? 'Permanent' : 'Temporaire'}
                                </Badge>
                                {ban.banType === 'temporary' && ban.expiryDate && (
                                  <Badge variant="outline" className="text-xs">
                                    <Clock className="w-3 h-3 mr-1" />
                                    Expire le {new Date(ban.expiryDate).toLocaleString('fr-FR')}
                                  </Badge>
                                )}
                              </div>
                              <p className="text-sm text-muted-foreground mb-2">
                                <strong>Raison:</strong> {ban.reason}
                              </p>
                              <div className="flex items-center space-x-4 text-xs text-muted-foreground">
                                <span>Banni le {new Date(ban.bannedAt).toLocaleDateString('fr-FR')}</span>
                            ...
                            <Button
                              ...
                              className="text-green-400 hover:text-green-300 border-green-400/30 hover:border-green-300/30"
                            >
                              Débannir
                            </Button>
                          </div>
