...
import { Plus, Palette, Trash2, Settings, Ban, AlertTriangle, MessageSquare, Crown } from 'lucide-react';
import { useLocalRoleSystem, type RolePermissions, type CustomRole } from '@/hooks/useLocalRoleSystem';
...
export function CustomRoleCreator() {
  ...
  const permissionsList = [
    { key: 'canAccessAdminPanel' as keyof RolePermissions, label: 'Accès admin panel', icon: Settings },
    { key: 'canManageForum' as keyof RolePermissions, label: 'Gérer le forum', icon: MessageSquare },
    { key: 'canWarnUsers' as keyof RolePermissions, label: 'Avertir des utilisateurs', icon: AlertTriangle },
    { key: 'canBanUsers' as keyof RolePermissions, label: 'Bannir des utilisateurs', icon: Ban },
    { key: 'canAssignRoles' as keyof RolePermissions, label: 'Assigner des rôles', icon: Crown },
    { key: 'canManageMaintenance' as keyof RolePermissions, label: 'Gérer la maintenance', icon: Settings },
  ];
            ...
            <div className="space-y-6">
                    ...
                    {PRESET_COLORS.map((color) => (
                      <button
                        ...
                        className={`w-6 h-6 rounded-full border-2 hover:scale-110 transition-transform ${
                          roleForm.color === color ? 'border-white' : 'border-border'
                        }`}
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3 mt-2">
                  {permissionsList.map(({ key, label, icon: Icon }) => (
                    <div key={key} className="flex items-center space-x-2 p-2 rounded-lg border border-border/50">
                      <Switch
                        id={key}
              ...
              <div className="flex justify-end space-x-2">
                ...
                <Button
                  ...
                  disabled={isSubmitting || !roleForm.name.trim() || !roleForm.displayName.trim()}
                >
      ...
      <div className="grid gap-4">
                ...
                <CardContent>
                  <div className="space-y-3">
                    <div className="text-xs text-muted-foreground">
                      Créé par {role.createdBy} le {new Date(role.createdAt).toLocaleDateString('fr-FR')}
                    </div>
        ...
        {customRoles.length === 0 && (
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p className="text-sm">Créez votre premier rôle personnalisé pour commencer</p>
          </div>
        )}
      </div>
    </div>
